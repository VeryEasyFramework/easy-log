...
import { EasyLogConfig, LogMessage, LogOptions, StackFrame } from "#/types.ts";
import { BasicFgColor } from "@vef/easy-utils/colorMe";
import formatUtils from "@vef/easy-utils/formatUtils";
import printUtils from "@vef/easy-utils/printUtils";
  ...
  private getCallFrame(): StackFrame {
    ...
    for (let i = 0; i < parts.length; i++) {
      if (parts[i].includes("EasyLog.log")) {
        return parseStackFrame(parts[i + 1]);
      }
    }
    return parseStackFrame(null);
  }
